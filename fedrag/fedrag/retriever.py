...
class Retriever:
    ...
    def build_faiss_index(self, dataset_name, batch_size=32, num_chunks=None):
        """Build a FAISS index for a given dataset.

        Args:
            dataset_name: Name of the dataset directory under data/corpus.
            batch_size: Number of documents per embedding batch.
            num_chunks: If provided, limit to the first N chunks for faster builds.
        """
    ...
    def query_faiss_index(self, dataset_name, query, knn=8):
        ...
        Args:
            dataset_name: Dataset directory name under data/corpus.
            query: Plain-text search query to embed and search with.
            knn: Number of nearest documents to retrieve.

        Returns:
            OrderedDict mapping doc_id to metadata including rank, score, title, content.
        """
        dataset_dir = os.path.join(CORPUS_DIR, f"{dataset_name}")
        index_path = os.path.join(dataset_dir, "faiss.index")
        doc_ids_path = os.path.join(dataset_dir, "all_doc_ids.npy")
    ...
    def index_exists(cls, dataset_name):
        """Return True if both FAISS index and doc IDs exist for the dataset."""
        dataset_dir = os.path.join(CORPUS_DIR, f"{dataset_name}")
        index_path = os.path.join(dataset_dir, "faiss.index")
        doc_ids_path = os.path.join(dataset_dir, "all_doc_ids.npy")
